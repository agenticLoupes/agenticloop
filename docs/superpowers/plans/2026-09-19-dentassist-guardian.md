...
- **Patient records are synthetic; imaging may be CC-BY real radiographs (user-approved realism upgrade).** Records come from **Synthea** (Apache-2.0, realistic synthetic) + hand-authored controlled demo patients (DEMO-007..010). Images: prefer a verified **CC-BY / CC0** labeled radiograph set (commercial-OK, attributed) — e.g. DenPAR; **DENTEX is rejected (CC-BY-NC-SA, non-commercial)**. If no clean CC-BY set is available, fall back to self-generated synthetic images. UI must always show `SYNTHETIC DATA — PROTOTYPE`; provenance + attribution documented in README (§18, Phase 6). Never imply real patient identities. **§18/§26 timebox: do not let data import become a project of its own — the agent loop is the priority.**
